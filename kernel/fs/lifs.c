...
void lifs_blk_iwrite(struct inode *inod, char *buffer, unsigned int blkadr) {
    ...
    for(int i = 0; i < strlen(buffer); i++) buf[i] = buffer[i];
...
void lifs_iwrite(struct inode *inod, char *buffer) {
    if(strlen(buffer) <= 512) {
        lifs_blk_iwrite(inod, buffer, 0);
        return;
    } else {
        printf("[lifs] Writing buffers longer than one block (512 bytes) is not yet implemented\n");
        return;
    }
}
...
void lifs_write(char *name, char *buffer) {
    ...
    for(int i = 0; i < LIFS_MAX_FILES; i++) {
        if(strcmp(lifs_inodes[i].name, name) == 0) {
            lifs_iwrite(&lifs_inodes[i], buffer);
            return;
        }
    }
